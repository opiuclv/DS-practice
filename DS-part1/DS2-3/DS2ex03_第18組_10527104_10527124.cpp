...
public:
	...
	*/
	void PrintRoot( treePtr root ) {                  				 // 印出樹的根 
		int index = 0, n = 1;
		while ( index < root -> K1Q ) {                              // 印出key1  
			cout << setw(3) << n << ": [" << root -> key1[index].serialNumber << "] ";
			PrintTheData( root -> key1[index].serialNumber );
	...
	void Search( string name ) {                                     // 尋找指定校名的key 
		treePtr local = a23tree;
		int index = 0, done = 0, n = 1;
		while ( done == 0 ) {                                        // 若尚未找到同名key 
			if ( name.compare( local -> key1[0].school ) < 0 ) {     // <key1 
				if( local -> left != NULL )
				    local = local -> left;
				else
				    done = 3;
			}
			else if ( name.compare( local -> key1[0].school ) == 0 ) // 同key1 
				done = 1;
			else {
				if ( local -> key2 != NULL) {                        // 若key2存在 
			   	 	if ( name.compare( local -> key2[0].school ) < 0 ) { // 在兩key之間 
					   	if ( local -> down1 != NULL )
			    	    	local = local -> down1;
						else
				   			done = 3;
					}
					else if ( name.compare( local -> key2[0].school ) == 0 ) // 同key2 
						done = 2;
					else {                                       	 // >key2 
				    	if ( local -> right != NULL )
    			     		local = local -> right;
			...
			if ( done == 1 ) {                                    	 // 同key1
				while ( index < local -> K1Q ) {
					cout << setw(3) << n << ": [" << local -> key1[index].serialNumber << "] ";
			    	PrintTheData( local -> key1[index].serialNumber );
			    	index++;
			    	n++;
				}
			}
			if ( done == 2 ) {                                    	 // 同key2 
				while ( index < local -> K2Q ) {
					cout << setw(3) << n << ": [" << local -> key2[index].serialNumber << "] ";
			    	PrintTheData( local -> key2[index].serialNumber );
			    	index++;
			    	n++;
				}
			}
...
void Mission1( List &aList ) {                                       // 任務1 
	...
	aList.PrintRoot( root );                                		 // 印出樹根 
	//aList.PrintPreOrder( root, 1 );                                // 印出2-3樹 
	
}

void Mission2( List &aList ) {                                       // 任務2
	string name;
	cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
	cout << "Mission 2: Search a 2-3 tree (Search key is college name)" << endl;
	cout << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << endl;
	cout << endl << "Enter a name to search:" << endl;
	cin >> name;
	aList.Search( name );
}
