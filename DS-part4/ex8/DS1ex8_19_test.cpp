...
void deleteSchoolName ( char obj[], node *ptr, node *root ){ // 刪除 
	...
	while(1) {
		...
		{
			if( strcmp ( ptr -> rnode -> fornode.schoolname, obj ) == 0  ) { // 有左右子樹
			    node *newNode = new node ; // 接ptr右邊左子樹最大的 
		...
		{
			if( strcmp ( ptr -> lnode -> fornode.schoolname, obj ) == 0  ) { // 有左右子樹
			    node *newNode = new node ; 
				newNode = ptr->lnode ; // 應該要等於佐子樹最大的 的後面一個 
				while( newNode->rnode ) {
