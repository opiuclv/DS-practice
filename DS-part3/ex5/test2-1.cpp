...
void inputSort( queue ) { // 讀入sort檔"一次一個job"

        char ch ;			
		fscanf( fptr, "%d%d%d%d", &queue.jobID2, &queue.arrival2, &queue.duration2, &queue.timeout2 ) ;
	...
	while ( true )
	{
		...
		if ( start == 0 )                                  // 結束 
		    ...
		    if ( input == 501 ) {
				...
				for( i = 0 ; i < 5 && abort( queue, i ); i++ )            //  讀入 
				{
		            inputSort( queue ) ; // 把資料讀進來	
				} // for
